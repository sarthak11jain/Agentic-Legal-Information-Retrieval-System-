#!/usr/bin/env python3
"""Apply the LLM law-citation filter to an existing submission file."""
...
SRC_DIR = Path(__file__).resolve().parents[5] / "src"
if str(SRC_DIR) not in sys.path:
    sys.path.insert(0, str(SRC_DIR))

from legal_rag.llm import (  # noqa: E402
    confirm_removals,
    parse_decisions as parse_public_decisions,
    parse_decisions_fallback as parse_public_decisions_fallback,
)
...
DEFAULT_SUBMISSION_PATH = BASE_DIR / "context/test_submission_tfidf_cosine_ce_weighted_top50.csv"
...
DEFAULT_OUTPUT_PATH = BASE_DIR / "context/test_submission_tfidf_cosine_ce_weighted_top50_llm.csv"
DEFAULT_VAL_OUTPUT_PATH = BASE_DIR / "context/val_submission_tfidf_cosine_ce_weighted_top50_llm.csv"
DEFAULT_PREDICTION_OUTPUT_PATH = (
    BASE_DIR / "context/test_llm_submission_rerank_predictions.csv"
)
DEFAULT_BATCH_OUTPUT_PATH = BASE_DIR / "context/test_llm_submission_rerank_batches.parquet"
DEFAULT_SUMMARY_OUTPUT_PATH = BASE_DIR / "context/test_llm_submission_rerank_summary.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LLM-filter an existing law submission.")
    parser.add_argument(
        "--dataset",
        choices=["test", "val"],
        default="test",
    ...
    parser.add_argument("--court-context-freeze-top-k", type=int, default=5)
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "0 auto-detects from vLLM model metadata; negative disables."
        ),
    )
    parser.add_argument(
        "--llm-token-buffer",
...
def parse_decisions(raw_response: str) -> tuple[dict[str, str], str]:
    return parse_public_decisions(raw_response)


def parse_decisions_fallback(raw_response: str) -> dict[str, str]:
    return parse_public_decisions_fallback(raw_response)
    ...
    *,
    ...
    for row in selected.itertuples(index=False):
        ...
        for rank_position, citation in enumerate(split_refs(row.predicted_citations), start=1):
            if rank_position <= args.court_context_freeze_top_k:
                continue
    ...
    *,
    ...
    for batch in parquet.iter_batches(
        ...
        for row in frame.itertuples(index=False):
            refs = list(dict.fromkeys(split_refs(row.law_references)))
            matched_refs = [ref for ref in refs if ref in target_citations]
            if not matched_refs:
                continue
            court_embedding = normalize_embedding(row.embedding)
            ...
            for citation in matched_refs:
                ...
                for query_id in citation_to_query_ids[citation]:
                    ...
                    context = {
                        "court_citation": str(row.citation),
                        ...
                        "document": truncate_text(row.document, args.court_context_max_chars),
                    }
    ...
    if args.court_context_confirm_removals and confirm_candidates:
        ...
        for result in confirm_results:
            ...
            for label, item in zip(LABELS, batch):
                ...
                first_pass_decision = candidate_state.get(citation, "KEEP")
                final_decision = confirm_removals(
                    {label: first_pass_decision},
                    {label: confirmation_decision},
                ).get(label, "KEEP")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        while futures:
            ...
            for future in done:
                ...
                completed_count += 1
                if completed_count % max_workers == 0 or completed_count == len(jobs):
...
def summarize(
    ...
    *,
    ...
    if gold_column and gold_column in queries.columns:
        original_scores = score_against_gold(submission, queries, gold_column=gold_column)
        final_scores = score_against_gold(output, queries, gold_column=gold_column)
...
def score_removal_quality(
    ...
    *,
    ...
    gold_by_query = {
        str(row.query_id): set(split_refs(getattr(row, gold_column)))
